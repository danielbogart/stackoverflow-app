...
var showAnswerer = function(answer) {
	...
	var answerElem = result.find('.answererName a');
	answerElem.attr('href', answer.link);
	answerElem.text(answer.title);

	// set the #posts for answer property in result
	var posts = result.find('.posts');
	posts.text(answer.post_count);

	// set some properties related to asker
	var answererName = result.find('.answererName');
	answererName.html('<a target="_blank" href=http://stackoverflow.com/users/' +answer.user.user_id + ' >' +
													answer.user.display_name + '</a>' + '</p>');
...
//add function for top answerers
